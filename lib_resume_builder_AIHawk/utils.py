    ...
    try:
        ...
        pdf_base64 = driver.execute_cdp_cmd(
            "Page.printToPDF",
            {
                ...
                "marginTop": 0,  # Margine superiore in pollici (circa 2 cm)
                "marginBottom": 0,  # Margine inferiore in pollici (circa 2 cm)
                "marginLeft": 0,  # Margine sinistro in pollici (circa 2 cm)
                "marginRight": 0,  # Margine destro in pollici (circa 2 cm)
...
def get_chrome_browser_options():
    ...
    options.add_argument(
        "--no-sandbox"
    )  # Disabilita la sandboxing per migliorare le prestazioni
    options.add_argument(
        "--disable-dev-shm-usage"
    )  # Utilizza una directory temporanea per la memoria condivisa
    options.add_argument(
        "--ignore-certificate-errors"
    )  # Ignora gli errori dei certificati SSL
    ...
    )  # Disabilita il throttling dei timer in background
    options.add_argument(
        "--disable-backgrounding-occluded-windows"
    )  # Disabilita la sospensione delle finestre occluse
    ...
    options.add_argument(
        "--no-first-run"
    )  # Disabilita la configurazione iniziale del browser
    options.add_argument(
        "--no-default-browser-check"
    )  # Disabilita il controllo del browser predefinito
    ...
    options.add_argument(
        "--disable-autofill"
    )  # Disabilita l'autocompletamento dei moduli
    ...
    )  # Esclude switch della modalità automatica e logging
