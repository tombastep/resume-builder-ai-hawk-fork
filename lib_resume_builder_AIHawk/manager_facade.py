...
class FacadeManager:
    def __init__(
        self, api_key, style_manager, resume_generator, resume_object, log_path
    ):
    ...
    def pdf_base64(self, job_description_url=None, job_description_text=None):
        if job_description_url is not None and job_description_text is not None:
            raise ValueError(
                "Esattamente uno tra 'job_description_url' o 'job_description_text' deve essere fornito."
            )
        ...
        with tempfile.NamedTemporaryFile(
            ...
            if job_description_url is None and job_description_text is None:
                ...
                self.resume_generator.create_resume_job_description_url(
                    style_path, job_description_url, temp_html_path
                )
            elif job_description_url is None and job_description_text is not None:
                self.resume_generator.create_resume_job_description_text(
                    style_path, job_description_text, temp_html_path
                )
            else:
                return None
